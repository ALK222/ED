...
void registraPaciente(CentroSalud &centro)
{
	...
	catch (EIncorporacionNoAdmitida)
	{
...
int main()
{
	...
	while (cin >> comando)
	{
		if (comando == "registraMedico")
			registraMedico(centro);
		else if (comando == "registraPaciente")
			registraPaciente(centro);
		else if (comando == "pideCita")
			pideCita(centro);
		else if (comando == "citados")
			citados(centro);
		else if (comando == "cancelaCita")
			cancelaCita(centro);
		else if (comando == "vinculados")
			vinculados(centro);
	}
	return 0;
}
