/**
 ...
 (c) Marco Antonio G�mez Mart�n, 2012;  Mercedes G�mez y Jos� Luis Sierra, 2017
     Modificado en 2022 por Jos� Luis Sierra para permitir declarar iteradores no inicializados.
*/
...
private:
    ...
    */
    static Nodo *borraRaiz(Nodo *p)
    {
        ...
        if (p->_iz == NULL)
        { // Si no hay hijo izquierdo, la ra�z pasa a ser el hijo derecho
            aux = p->_dr;
            delete p;
            return aux;
        }
        else if (p->_dr == NULL)
        { // Si no hay hijo derecho, la ra�z pasa a ser el hijo izquierdo
            aux = p->_iz;
            delete p;
            return aux;
        }
        else
        { // La nueva ra�z es el elemento m�s peque�o del hijo derecho (sucesor de la actual ra�z)
    ...
    */
    static Nodo *mueveMinYBorra(Nodo *p)
    {
        ...
        //
        if (padre != NULL)
        {                          // el sucesor no es la ra�z del subarbol derecho sino un nodo m�s profundo
        ...
        { // el sucesor es la ra�z del subarbol derecho de la clave a borrar
            aux->_iz = p->_iz;
        }
