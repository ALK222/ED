...
public:
	void enredar()
	{
		// OPERACION A IMPLEMENTAR
		if (_prim != nullptr)
		{
			Nodo *act = this->_prim;

			while (act->_sig != _ult && act != _ult)
			{
				Nodo *aux = _ult; // ultimo actual
				_ult = _ult->_ant;
				_ult->_sig = nullptr;
				aux->_sig = act->_sig; // el siguiente del auxiliar es el siguiente del actual
				act->_sig->_ant = aux;
				act->_sig = aux; // el siguiente del actual es el auxiliar
				aux->_ant = act;
				act = aux->_sig;
			}
		}
...
private:
	...
	*/
	...
	/*void intercambia(Nodo* n1, Nodo* n2)
	{
		n1->_sig = n2->_sig;
		n2->_ant = n1->_ant;
		n2->_sig = n1;
